...
\noindent {\bf Learning Targets:} For Checkpoint \#12, there are four learning target being assessed: 
...
\noindent To demonstrate competency on Learning Target \#$n$, you need to correctly respond to all or nearly all of the prompts in question \#$n$.  If you have already passed a learning target on a prior Checkpoint, you don't need to re-attempt it on this one.
  ...
  \begin{enumerate}
	...
	\item[12.] 	Suppose we want to build a rectangular box with the following properties:  the box has a rectangular base whose length is always twice its width, the box is open (no top), and the base material costs \$5 per square foot while the material for the sides costs \$1.50 per square foot.  What is the least expensive box we can build if the box must hold $8$ cubic feet of volume?
	
	%Suppose we want to build a cylindrical can with no top in such a way that its surface area is exactly $4$ square feet.  What dimensions produce the can having absolute maximum total volume?  What is the absolute minimum volume that is possible?
...
\pagebreak
	...
	\item[13.] An object is moving along a straight path; its velocity is considered positive when moving to the right. The object's velocity $v(t)$ (in feet per second) at time $t$ (in seconds) is given by the graph above. 	

	\begin{figure}
	\begin{center}	
	   \scalebox{0.35}{\includegraphics{images-W25/CP12-velocity.jpg}}
	...
	\be
		...
		\item On the time interval $[3,7]$, determine the object's change in position, including units on your result.  Clearly explain your thinking and process for determining the change in position.
		
		\lr \tr
		
		\item On the time interval $[3,7]$, determine the object's total distance traveled, including units on your result.  Is the distance traveled on this interval the same as the object's change in position?  why or why not? Clearly explain your thinking and process for determining the  distance traveled.
		
		\lr \tr
		
		\item Given an example of an interval $[a,b]$ on which the object's distance traveled is exactly 4 feet (that is, identify numerical values for $a$ and $b$).  Write a sentence to explain and justify your choice of interval.
		
	\ee

\pagebreak

	\item[14.] A person is jogging along a long straight path that runs from west to east; their velocity is considered positive when jogging east.  At different points in time, their velocity is recorded, and some of the data is given in the following table.
	
	\begin{center}	
	\begin{tabular}{c|ccccccc}
	 $t$ (seconds) & 0 & 5 & 10 & 15 & 20 & 25 & 30 \\
	 \hline
	 $v(t)$ (feet per second) & 10.2 & 10.5 & 10.7 & 10.4 & -9.9 & -10.1 & -10.3 \\ 
	\end{tabular}
\end{center}

	\be
		\item On the time interval $[0,15]$, determine $R_3$, the right Riemann sum for the function $v(t)$ using 3 subintervals.  Clearly show your computations below with proper Riemann sum notation.
		
		\mr
		
		\item In the context of the given information, what is the meaning of $R_3$ that you computed in (a)?
		
		\lr
		
		\item Now, on the time interval $[0, 30]$, determine $M_3$, the middle Riemann sum for $v(t)$ using 3 subintervals.  Again, clearly show your computations using proper notation.
		
		\mr
		
		\item In the context of the given information, what is the meaning of $M_3$ that you computed in (c)?
	\ee
...
\item[\#15.] Consider the piecewise linear function $y = f(x)$ pictured below.
	...
	\begin{center}
		\scalebox{0.45}{\includegraphics{images-W25/CP-11-14-11.png}}
	\end{center}
